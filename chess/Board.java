...
public class Board {
	...
	public static boolean movePiece(int i0, int j0, int i1, int j1, Piece p){
		// If there is a piece on the destination coords
		if(board[i1][j1].hasPiece() == true)
		{
			// To check if any king has been captured
			if(board[i1][j1].getPiece().getSymbol() == "♚" || board[i1][j1].getPiece().getSymbol() == "♔")
			{
				System.out.print(board[i0][j0].getPiece().getColour());
			...
			// To check if any piece wants to kill another piece 
			else
			{
				board[i1][j1].setPiece(p);
				board[i0][j0].removePiece();
		...
		// If no piece wants to kill another piece 
		else
		{
			board[i1][j1].setPiece(p);
			board[i0][j0].removePiece();
